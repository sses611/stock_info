var REQUEST_URL = 'http://apis.data.go.kr/1160100/service/GetStocDiviInfoService/getDiviInfo' //주식배당정보 - 배당정보조회
// var REQUEST_URL = 'http://apis.data.go.kr/1160100/service/GetStockSecuritiesInfoService';
...
function GetStockList(){
    ...
    LoadData(REQUEST_URL + "?pageNo=1&numOfRows=1&resultType=json&stckIssuCmpyNm=" + encodeURI(StockName) + "&scrsItmsKcdNm=" + encodeURI(StockType) + "&serviceKey=" + serviceKeyStr, 0);
}
...
function ParseJSON1(data) {
    ...
    if (item === undefined)
        ...
        `<li>기준일자<span>${ParseDate(item.basDt)}</span></li>` +
        `<li>발행주식<span>${new Intl.NumberFormat().format(item.issuStckCnt)}주</span></li>` +
...
function LookupAllStock(){
    window.open('http://www.naver.com/stock');
}

function LookupPrevData() {
    console.log('LookupPrevData');
    // if (9 > stockIndex) stockIndex += 1;
    // else stockIndex = 9;
    // ParseJSON2(stockData, stockIndex);
}

 function LookupNextData() {
    console.log('LookupNextData');
